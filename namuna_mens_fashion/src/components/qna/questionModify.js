    ...
    return(
        <form className='qnas-write-form' method="POST" onSubmit={(e) => test_submit(e, title, author, content)} action={`http://localhost:${process.env.REACT_APP_SERVER_PORT}/qnas/write`}>
            <Table className='table-bordered'>
                ...
                <tr>
                    <td>제목</td>
                    <td><input id='question-title' className={`form-control border ${titleValid}`} type='text' name='title' value={question.title}/></td>
                </tr>
                <tr>
                    <td>작성자</td>
                    <td><input id='question-author' className={`form-control border ${authorValid}`} type='text' name='author' value={question.author}/></td>
                </tr>
                <tr>
                    <td>내용</td>
                    <td><textarea id='question-content' className={`form-control border ${contentValid}`} name='content' rows="10" value={question.content}/></td>
                </tr>
            </Table>
